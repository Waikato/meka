...
Evaluating incremental classifiers will carry out evaluation for the data over $\texttt{B}-1$ evaluation windows (an initial window is used for initial training but is not included in evaluation). The result shown is averaged across all windows. On the command line, the per-window evaluation statistics are also shown. %Note that the \framework{\MOA} framework \cite{MOA} for evaluating incremental classifiers is much more sophisticated; and a new release will include a wrapper to \framework{\MEKA}. 
