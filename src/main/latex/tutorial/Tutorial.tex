...
Note that the \texttt{IncrementalEvaluation} class is called for evaluation in this case; see Section \ref{sec:evaluation}. The \framework{\MOA} framework \cite{MOA} for learning in data streams contains a number of incremental classifiers that \WEKA{} does not, for example Hoeffding trees. You can run them using \WEKA's \texttt{MOA} meta classifier (wrapper)\footnote{\framework{\MOA} does not come in the \framework{\MEKA} package, you can download it from \url{http://moa.cs.waikato.ac.nz/}}, e.g., 
\begin{verbatim}
...
\begin{tabular}{ll}
	...
	Probabilistic Classifier Chains [$M$] & \texttt{PCC} \\
	Monte Carlo Optimization of CC [$M$] & \texttt{MCC} \\
	\hline
	...
	\hline
	Label Combination (LC, aka LP) & \texttt{CDN} \\ 
