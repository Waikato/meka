...
public class DataTable extends JTable {
	...
	protected class RelationalCellEditor extends AbstractCellEditor implements
			...
			m_Button.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					dialog.setTitle("Relational attribute Viewer - "
						+ ((DataSortedTableModel) getModel()).getInstances()
		...
		protected Instances getInstancesAt(int rowIndex, int columnIndex) {
			...
			model = (DataSortedTableModel) getModel();
	...
	public void setModel(TableModel model) {
		...
		if (!(model instanceof DataSortedTableModel)) {
			return;
		}

		arffModel = (DataSortedTableModel) model;
	...
	public TableCellEditor getCellEditor(int row, int column) {
		...
		if ((getModel() instanceof DataSortedTableModel)
			&& (((DataSortedTableModel) getModel()).getType(column) == Attribute.RELATIONAL)) {
	...
	public boolean isReadOnly() {
		return ((DataSortedTableModel) getModel()).isReadOnly();
	}
	...
	public void setReadOnly(boolean value) {
		((DataSortedTableModel) getModel()).setReadOnly(value);
	}
	...
	private void setLayout() {
		...
		for (i = 0; i < getColumnCount(); i++) {
			...
			getColumnModel().getColumn(i)
				.setCellRenderer(new DataTableCellRenderer());
