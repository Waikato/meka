 ...
 */
public class MCCtemp extends MultilabelClassifier implements Randomizable, TechnicalInformationHandler { 
	...
	public static double[] RandomSearch(CCe h, Instance x, int T, Random r, double y0[]) throws Exception {
		...
		// pre-transform instances
		Instance x_copy[] = h.transformInstance(x);

		y = Arrays.copyOf(y,y.length);
		for(int t = 0; t < T; t++) {
			// propose y' by sampling i.i.d.
			double y_[] = h.sampleForInstance(x_copy,r); 	       
			// rate y' as w'
	...
	public static void main(String args[]) {
		MultilabelClassifier.evaluation(new MCCtemp(),args);
	}
